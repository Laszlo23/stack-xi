...
import { proxyZeroXPrice, isZeroXConfigured } from "@/lib/swap/zerox-proxy";
...
export const Route = createFileRoute("/api/swap/price")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        const rateLimited = checkRateLimit(request, { routeId: "swap-price", maxPerWindow: 40 });
        ...
        try {
          const data = await proxyZeroXPrice({
          ...
          const message = err instanceof Error ? err.message : "0x price failed";
